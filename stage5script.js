...
class SnakeGame {
    ...
    clearBoard() {
        ...
        backgroundImage.src = 'depositphotos_389158500-stock-photo-space-aircrafts-universe-fantasy-backdrop.jpg'; // Draw the background image on the canvas
    ...
    replaceSnakeBodyWithExplosion() {// Loop through the snake body letters and replace invalid word letters with '💥'
        for (let i = 0; i < this.snakeBodyLetters.length; i++) {
            if (i + 2 < this.snakeBodyLetters.length && this.snakeBodyLetters.slice(i, i + 3).some(letter => letter === null)) {// Replace these letters with '💥'
                for (let j = i; j < i + 3; j++) {
